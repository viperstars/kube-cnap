...
func NewK8sTestClients(logger *zap.Logger, service DBService) *Clients {
    client := new(Clients)
    client.logger = logger
    clients := make(map[string]map[string]*Client)
    client.ClientPool = clients
    config, _ := clientcmd.RESTConfigFromKubeConfig([]byte(Config))
...
func NewK8sClients(logger *zap.Logger, service DBService) (*Clients, error) {
    client := new(Clients)
    client.logger = logger
    client.db = service
    err := client.LoadFromDB()
    return client, err
}
